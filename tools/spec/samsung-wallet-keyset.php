...
/**
 * Usage:
 *   php samsung-wallet-keyset.php generate   — print JSON keyset from src/Pass/Samsung/Model phpstan shapes
 *   php samsung-wallet-keyset.php check      — compare generate() to tools/spec/samsung-wallet-keyset.json (exit 1 if diff)
 *   php samsung-wallet-keyset.php baseline   — overwrite samsung-wallet-keyset.json with generate() output
 */

$cmd = $argv[1] ?? '';

if (!\in_array($cmd, ['generate', 'check', 'baseline'], true)) {
    fwrite(STDERR, "Usage: php samsung-wallet-keyset.php generate|check|baseline\n");
    exit(2);
}

$modelDir = dirname(__DIR__, 2) . '/src/Pass/Samsung/Model';
$keysetPath = __DIR__ . '/samsung-wallet-keyset.json';

/** @var array<string, array{0: string, 1: string}> */
$entities = [
    'card' => ['Card.php', 'CardEnvelopeType'],
    'cardData' => ['CardData.php', 'CardDataType'],
    'localization' => ['Localization.php', 'LocalizationType'],
    'samsungImage' => ['Shared/SamsungImage.php', 'SamsungImageType'],
    'samsungBarcode' => ['Shared/SamsungBarcode.php', 'SamsungBarcodeType'],
    'location' => ['Shared/Location.php', 'SamsungLocationType'],
    'boardingPassAttributes' => ['BoardingPass/BoardingPassAttributes.php', 'BoardingPassAttributesType'],
    'eventTicketAttributes' => ['EventTicket/EventTicketAttributes.php', 'EventTicketAttributesType'],
    'couponAttributes' => ['Coupon/CouponAttributes.php', 'CouponAttributesType'],
    'giftCardAttributes' => ['GiftCard/GiftCardAttributes.php', 'GiftCardAttributesType'],
    'loyaltyAttributes' => ['Loyalty/LoyaltyAttributes.php', 'LoyaltyAttributesType'],
    'genericAttributes' => ['Generic/GenericAttributes.php', 'GenericAttributesType'],
    'digitalIdAttributes' => ['DigitalId/DigitalIdAttributes.php', 'DigitalIdAttributesType'],
    'payAsYouGoAttributes' => ['PayAsYouGo/PayAsYouGoAttributes.php', 'PayAsYouGoAttributesType'],
];
...
if ($normBaseline === $normGenerated) {
    echo "OK: Samsung wallet keyset matches baseline.\n";
    exit(0);
}

echo "Samsung wallet keyset mismatch (entities => sorted JSON keys from phpstan array shapes).\n";
